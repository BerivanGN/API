...
public class C17_BaseUrlHerokuappQueryParam extends HeroquAppBaseUrl {
    ...
    */
     ...
     */
        ...
        Response response = given().spec(specHeroquApp).when().get("/{pp1}");
    ...
    */
        // 1 - Url hazırla
        specHeroquApp.pathParam("a","booking").queryParam("firstname","Eric");

        // 2 - Expected Data hazırla

        // 3 - Response'ı kaydet

        Response response = given().spec(specHeroquApp).when().get("/{a}");
        response.prettyPrint();
